...
VGDonors <- filter(df, grepl("Valley Gives", df$`Campaign Name`))

#All donations not to Valley Gives
notVGDonors <- filter(df, !grepl("Valley Gives", df$`Campaign Name`))
...
VGPlusDonors <- subset(df, `Account ID` %in% VGPlusIDs)
